"""Apply manual entity corrections (deletions and relabellings) to a NER manifest."""
...
def main():
    ap = argparse.ArgumentParser(description="Apply manual entity corrections to a NER manifest.")
    ap.add_argument(
        "--manifest_in",
        default="data/processed/manifests/ner_manifest_mapped.jsonl",
        help="Input NER manifest (JSONL)",
    )
    ap.add_argument(
        "--manifest_out",
        default="data/processed/manifests/ner_manifest_mapped_fixed.jsonl",
        help="Output corrected NER manifest (JSONL)",
    )
    args = ap.parse_args()
    ...
    manifest_index = {}
    for row in iter_jsonl(manifest_in):
